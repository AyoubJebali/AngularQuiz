...
def get_quizzes():
    ...
    selected_categories = [
        category for category in data if category["id"] in category_ids
    ]
    ...
    for category in selected_categories:
        questions.extend(
            category["questions"]
        )  # Assuming 'questions' is a list within each category

    # Check if there are enough questions
    if len(questions) < 10:
        return (
            jsonify({"error": "Not enough questions in the selected categories"}),
            400,
        )
