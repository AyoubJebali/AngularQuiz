    ...
    try:
        ...
        if not username or not password:
            current_app.logger.warning(
                f"Registration attempt with missing fields - username: {bool(username)}, password: {bool(password)}"
            )
...
def login():
    ...
    try:
        ...
        if not username or not password:
            current_app.logger.warning(
                f"Login attempt with missing fields - username: {bool(username)}, password: {bool(password)}"
            )
            return jsonify({"error": "Username and password are required"}), 400
