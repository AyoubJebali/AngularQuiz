...
def assert_valid_quiz_response(response_data):
    ...
    for question in response_data:
        ...
        assert isinstance(
            question["correctIndex"], int
        ), "correctIndex should be an integer"
        assert (
            0 <= question["correctIndex"] < len(question["options"])
        ), "correctIndex should be valid"
...
def assert_valid_user_response(response_data, expected_username=None):
    ...
    if expected_username:
        assert (
            response_data["username"] == expected_username
        ), f"Expected username {expected_username}"
...
def mock_quiz_data(num_questions=10, category_id=1, category_name="Test Category"):
    ...
    return [{"id": category_id, "name": category_name, "questions": questions}]
